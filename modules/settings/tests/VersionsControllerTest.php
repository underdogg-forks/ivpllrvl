...
class VersionsControllerTest extends TestCase
{
    ...
    {
        // Arrange
        $actions = $this->discoverPublicControllerActions(VersionsController::class);

        // Act
        $routes = array_map(fn (string $action): array => $this->guessRouteForControllerAction(VersionsController::class, $action), $actions);
    ...
    public function it_uses_post_routes_for_ajax_controller_actions(): void
    {
        ...
        // Assert
        if (!str_contains(VersionsController::class, 'AjaxController')) {
    ...
    {
        // Arrange
        $controllerReflection = new \ReflectionClass(VersionsController::class);
