...
class SettingsAjaxControllerTest extends TestCase
{
    ...
    {
        // Arrange
        $actions = $this->discoverPublicControllerActions(SettingsAjaxController::class);

        // Act
        $routes = array_map(fn (string $action): array => $this->guessRouteForControllerAction(SettingsAjaxController::class, $action), $actions);
    ...
    public function it_uses_post_routes_for_ajax_controller_actions(): void
    {
        ...
        // Assert
        if (!str_contains(SettingsAjaxController::class, 'AjaxController')) {
    ...
    {
        // Arrange
        $controllerReflection = new \ReflectionClass(SettingsAjaxController::class);
